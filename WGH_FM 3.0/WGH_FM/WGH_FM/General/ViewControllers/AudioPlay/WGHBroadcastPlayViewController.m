...
- (void)requestRecommendData {
    ...
    if (self.isRecommend == YES) {
        url = [NSString stringWithFormat:@"http://live.ximalaya.com/live-web/v1/getProgramDetail?device=iPhone&programScheduleId=%d&radioId=%d",[self.playModel.programScheduleId intValue],[self.playModel.radioId intValue]];
    }
    else
    {
        url = [NSString stringWithFormat:@"http://live.ximalaya.com/live-web/v1/getProgramDetail?device=iPhone&programScheduleId=0&radioId=%d",[self.playModelRecommand.radioId intValue]];
    }
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if (self.isRecommend == NO) {
        ...
        //插入推荐电台
        result = [db insertBroadcastHistoryRecommend:self.playModelRecommand];
    }
    else
    {
        //其他电台
        ...
        //插入其他电台
        result = [db insertBroadcastHistoryTop:self.playModel];
