...
- (void)requestAnchorAlbumListWithURL:(NSString *)urlStr block:(void (^)(NSMutableArray *))block {
    ...
    [manger GET:urlStr parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        self.dataArray=[NSMutableArray array];
        for (NSDictionary *dict in array) {
            WGHHotAnchorModel * anchorModel = [WGHHotAnchorModel new];
            [anchorModel setValuesForKeysWithDictionary:dict];
            [self.dataArray addObject:anchorModel];
        }
