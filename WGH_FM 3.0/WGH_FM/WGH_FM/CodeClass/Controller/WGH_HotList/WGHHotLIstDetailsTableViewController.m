...
- (void)requestOneHotListData {
    ...
    if ([self.model.contentType isEqualToString:@"track"]) {
        ...
        [[WGHRequestData shareRequestData] requestAnchorAlbumListWithURL:url block:^(NSMutableArray *array) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.model.contentType isEqualToString:@"track"]) {
        ...
        WGHHotAnchorModel *model = self.dataArray[indexPath.row];
        cell.numberLabel.text = [NSString stringWithFormat:@"%d",(int)(indexPath.row+1)];
        if (indexPath.row == 0) {
            cell.numberLabel.textColor = [UIColor orangeColor];
        }else if (indexPath.row == 1) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.model.contentType isEqualToString:@"album"]) {
        // 跳转专辑界面
        ...
        WGHHotAnchorAlbumViewController *anchorVC = [WGHHotAnchorAlbumViewController new];
        anchorVC.anchorModel = self.dataArray[indexPath.row];
        
        [self presentViewController:anchorVC animated:YES completion:nil];
