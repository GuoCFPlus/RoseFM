...
-(void)setHotDetailsModel:(WGHShowListModel *)hotDetailsModel
{
    if (_hotDetailsModel !=nil) {
        _hotDetailsModel = nil;
        _hotDetailsModel = hotDetailsModel;
    }
   [self.leftImgView sd_setImageWithURL:[NSURL URLWithString:hotDetailsModel.albumCoverUrl290] placeholderImage:[UIImage imageNamed:@"placeholder"]];
    self.mainTitleLabel.text=hotDetailsModel.title;
    self.subTitleLabel.text=hotDetailsModel.intro;
    self.playsCountsLabel.text = [NSString stringWithFormat:@"%.1f万",[hotDetailsModel.playsCounts floatValue]/10000];
    self.tracksCountsLabel.text = [NSString stringWithFormat:@"%d集",[hotDetailsModel.tracksCounts  intValue]];
}
