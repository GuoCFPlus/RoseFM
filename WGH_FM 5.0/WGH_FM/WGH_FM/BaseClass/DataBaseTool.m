...
- (void)createDownLoadList {
    ...
    // 准备SQL语句
    NSString *sqlWord = @"create table if not exists DownLoadList (\
    trackId INTEGER not null,\
    uid INTEGER,\
    playPathAacv164 text not null,\
    downloadAacUrl text,\
...
- (int)insertDownloadName:(WGHAlbumListModel *)model {
    
    //设置错误码
    int result = 0;
    //拼写执行的sql
    NSString *sql = @"insert into DownLoadList (trackId,\
    uid,\
    playPathAacv164,\
    downloadAacUrl,\
    title,\
...
- (NSMutableArray *)selectAllDownload:(NSString *)condition {
    NSMutableArray *array = [NSMutableArray array];
    //准备伴随指针
    sqlite3_stmt *stmt = nil;
    ...
    ID,\
    playsCounts,\
    sharesCounts  from DownLoadList where ? ";
