...
#pragma mark -----闹铃-----
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        if (self.alarmClock.on == YES) {
            ...
            WGHClockSetTableViewController *clockSetVC = [[WGHClockSetTableViewController alloc]initWithStyle:UITableViewStyleGrouped];
            [self.navigationController pushViewController:clockSetVC animated:YES];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex == 1) {
        ...
        [WGHSettingTableViewController registerLocalNotification:5];
...
+ (void)registerLocalNotification:(NSInteger)alertTime
{
    ...
    // 弹出的通知内容
    notification.alertBody = @"蔷薇FM通知您";
    // 气泡个数
    ...
    // 通知的参数
    NSDictionary * userDict = [NSDictionary dictionaryWithObject:@"蔷薇FM通知您" forKey:@"notification"];
